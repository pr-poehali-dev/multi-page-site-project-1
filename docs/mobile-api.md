...
### Автоматические push-уведомления (уже подключены на сайте)

Сайт сам отправляет уведомления в следующих случаях — от приложения ничего дополнительно не требуется, только чтобы `pushToken` был сохранён (см. пункт 1 выше):

1. **Смена статуса заявки** (одобрена / отклонена / возвращена на доработку) — уведомление уходит участнику, который подавал заявку, сразу после того как администратор меняет статус в админке. `data`: `{ "screen": "MyApplications", "applicationId": 123, "contestId": 42 }`.

2. **Публикация нового конкурса** — уведомление уходит ВСЕМ участникам с сохранённым push-токеном, сразу после того как администратор создаёт конкурс в админке. `data`: `{ "screen": "FestivalDetail", "contestId": 42 }`.

3. **Ручная рассылка из админки** — организатор может со страницы «Push-уведомления» в админ-панели отправить произвольное уведомление всем участникам, по желанию прикрепив ссылку на конкретный конкурс (тогда `data` тоже будет содержать `{ "screen": "FestivalDetail", "contestId": ... }`).

Во всех трёх случаях экран `FestivalDetail`/`MyApplications` в `data` — это подсказка, на какой экран должно перейти приложение при нажатии на уведомление; сама логика навигации реализуется в приложении.
